...
void TIMA0_PWM_init(uint8_t pin, uint32_t period, uint32_t prescaler, double percentDutyCycle, uint32_t clk_div) {
	...
	//->IOMUX pin stuff
	//Select PINCM25 for use as port I/O bc TIMA0 is PB28
	...
	TIMA0->COUNTERREGS.CC_01[pin] = 1;//TIMA0 channel 0 set to compare
	//Set TIMx.CCCTL_xy[0/1].COC = 1 for compare mode
	//	-> index chooses channel and val at index determines capture or compare mode
	TIMA0->COUNTERREGS.CCCTL_01[pin] = 1;//TIMA0 channel 0 needs control for compare
	//Set TIMx.CCCTL_xy[0/1].COC = 1 for compare mode
	...
	TIMA0->COUNTERREGS.CCACT_01[pin] = GPTIMER_CCACT_01_ZACT_MASK | GPTIMER_CCACT_01_LACT_MASK | GPTIMER_CCACT_01_CUACT_CCP_LOW | GPTIMER_CCACT_01_FENACT_MASK | GPTIMER_CCACT_01_SWFRCACT_MASK;
