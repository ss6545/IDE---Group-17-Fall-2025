...
void TIMA0_PWM_init(uint8_t pin, uint32_t period, uint32_t prescaler, double percentDutyCycle) {
	...
	//left en 
	IOMUX->SECCFG.PINCM[44] |= (IOMUX_PINCM_PC_CONNECTED | IOMUX_PINCM45_PF_GPIOB_DIO19 | IOMUX_PINCM_PIPU_ENABLE);
	GPIOB->DOESET31_0 |= (GPIO_DOE31_0_DIO19_ENABLE);
	GPIOB->DOUT19_16 |= GPIO_DOUT19_16_DIO19_ONE;   // drive pin high
	//right en
	IOMUX->SECCFG.PINCM[46] |= (IOMUX_PINCM_PC_CONNECTED | IOMUX_PINCM47_PF_GPIOA_DIO22 | IOMUX_PINCM_PIPU_ENABLE);
	GPIOA->DOESET31_0 |= (GPIO_DOE31_0_DIO22_ENABLE);
	GPIOA->DOUT23_20 |= GPIO_DOUT23_20_DIO22_ONE;   // drive pin high
...
void TIMA0_PWM_DutyCycle(uint8_t pin, uint32_t period, double percentDutyCycle) {
	// old calc		(((double)period)*(1.0 - ((percentDutyCycle)/100.0))  )
//uint32_t test = period - (period*((uint32_t) percentDutyCycle)) / 100;
//return test;
